---
name: gen-map-prompt
description: 用問答收集地圖需求，組出「手繪畫面地圖」的產圖 prompt（純文字，直接貼到 John 的產圖工具）。當 John 說「產地圖 prompt」「幫我出一張地圖的說明」「我要產 M4/新地區的圖」「地圖產圖問答」等需求時使用。規格權威來源是 docs/design/美術規格/地圖產圖規格.md（本 skill 內嵌的基礎規則區塊須與其同步）。
---
...
固定開頭（基礎規則，**與 地圖產圖規格.md 同步，改規格要兩邊一起改**）：
...
- 產完自查 地圖產圖規格.md 的交付檢查清單（尺寸整除 32／圖外深灰／開口對位／別畫互動物件）。
