...
本目錄是《水晶傳說》**Godot 版主專案**，POC 已可玩。遊戲規則的權威來源＝`build_cq2.py`（規則定義腳本）、資料種子＝`CONTENT.json`、系統邊界說明＝`DEV_開發指南.md`，三者收在本 repo 的 **`reference/gdevelop/`** 唯讀快照（2026-07-16），供開發對照（改規則／公式／存檔邏輯時回頭核對）。

> 所有 subagent 協作開工前都要先讀這份文件。
...
├── docs/                      # 文件中樞：design/(長什麼樣) pipeline/(怎麼產生) story/(世界觀)；索引 docs/README.md
│                              #   戰鬥立繪產線＝docs/pipeline/battle_art/（入口 workflow.md；gen-battle-prompt skill 引用）
├── specs/                     # 從 build_cq2.py「抄錄」出的權威規格（凍結版，對照實作用）
...
├── reference/legacy_art/      # 封存的舊美術文件（LPC製作流程 等）；唯讀、AI 不參考
...
- **數值資料真相源＝Godot 端 `resources/content/**/*.tres`**（2026-07-14 定案，取代早期「以 `CONTENT.json` 為源、run-time parse JSON」的做法）：party/equipment/skills/items/enemies/
...
- **`reference/gdevelop/build_cq2.py`**（凍結快照）是遊戲規則的唯一真相來源（規則邏輯都在這支腳本產生的 JsCode 裡）。`specs/` 下的規格文件是從這支
  腳本**凍結抄錄**出來的快照，抄錄時必須附原始行號；spec 引用的行號皆對照此檔。
- **美術/音效資產**已全數複製進 `godot-project/assets/`，以 repo 內為準，不重新繪製
  （除非任務清單明確標記為「順便重繪」）。素材出處與授權見 `CREDITS_素材授權.md`。
...
| 素材（`godot-project/assets/`）| `CREDITS_素材授權.md`（授權帳本）、必要時 `docs/pipeline/素材管理規範.md`|
...
```
- Spec/Task 版本: vX.Y
- 對應規則來源快照: build_cq2.py @ <git commit / 日期>
- 狀態: 草稿 / 定案 / 實作中 / 已驗收
```
...
- **引擎版本：Godot 4.7（stable，2026-06-18 釋出）**，不用 4.0-4.2（TileMap → TileMapLayer 破壞性 API 變動已在
  4.3 定型）。本專案原訂 4.3，2026-07-14 趁專案早期、程式碼尚少時把目標版本上修到 4.7，換取 4.4~4.7 累積的
...
- **語言：GDScript**（不用 C#）——理由：既有規則邏輯本來就是動態語言(JS)風格、資料驅動；GDScript 生態與
...
- **不要**把玩法邏輯寫成「每幀整段重跑」的巨大函式（那是早期 JsCode 每幀重跑的權宜作法，不是 Godot 該有的寫法）。
...
4. 遇到規格與現有 `specs/` 衝突（例如發現 `build_cq2.py` 與 DEV_開發指南.md 描述不一致），以 **`build_cq2.py` 實際邏輯為準**，回頭更新 spec 並註記版本遞增，不要自行改規則。
5. `reference/gdevelop/` 是唯讀凍結快照，**不要修改**。
