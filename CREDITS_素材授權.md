...
- `assets/battle/hero_*.png`：除下列明列的 AI idle 例外外，由 LPC 合成角色幀放大裁切（沿用 LPC CC-BY-SA/GPL 授權），
  ...
  - `hero_marin_idle_0-3` / `hero_marin_attack_0-4`（2026-07-27 整合）：Marin 重製的二頭身戰鬥 idle／attack strip 切幀，AI 生成（imagegen）＋去背，經 John 驗收；來源 strip 於 `assets-source/role/main/marin/`（`battle_idle_*`／`battle_attack_*`）。取代 2026-07-20 的舊 `hero_marin_f0-3`。提示詞作者 John／協作 Agent。
  - `hero_ludo_idle_0-3`（2026-07-27 整合）：Ludo 重製 idle 切幀（AI 生成，來源 `assets-source/role/main/ludo/ludo_battle_idle_*`）。**2026-07-27 再修**：原四幀切圖腳部水平漂移（idle 時像在平移），以「腳部中心對齊畫布中心」重新水平置中消除。`hero_ludo_attack_0-4`（2026-07-27 整合）：Ludo 重製的 `sword_diagonal_slash` 五幀 strip，AI 生成＋去背＋腳底錨點正規化，已取代舊 LPC slash 六幀。
  - `hero_alan_idle_0` / `hero_alan_attack_0-4`（2026-07-27 整合）：Alan 重製的二頭身戰鬥圖，AI 生成（imagegen）＋去背，經 John 驗收。idle 為**單張靜態**（來源 `assets-source/role/main/alan/battle_idle_alpha.png`，尚無 idle strip）；attack 為 `battle_attack_*` strip 切幀（5 幀）。取代先前暫用的舊 LPC idle。提示詞作者 John／協作 Agent。
  ...
  - Alan 戰鬥 `idle / calm_ready` 首幀（2026-07-27，候選 2）：OpenAI 內建 imagegen 依已驗收 Alan seed 與單手劍 weapon reference 生成，經 John 驗收並以洋紅鍵去背為 `assets-source/role/main/alan/battle_idle_alpha.png`（鍵圖保留為 `battle_idle_raw.png`）。此圖為 idle strip 的首幀造型錨點，尚未整合為 runtime 動畫；提示詞作者 John／協作 Agent。
  ...
  - Ludo 戰鬥 `attack / sword_diagonal_slash` 五幀 strip（2026-07-27）：OpenAI 內建 imagegen 依已驗收 Ludo seed、單手劍 weapon reference 與 attack 首幀生成；John 驗收靜態逐幀與 GIF 後，以螢光綠鍵去背、soft matte＋despill 清邊，並依腳底 bottom-center 錨點正規化。來源保存於 `assets-source/role/main/ludo/battle_attack_strip_{raw,alpha}.png`、`battle_attack_0..4.png`、`battle_attack_review_montage.png`，runtime 為 `hero_ludo_attack_0..4.png`；提示詞作者 John／協作 Agent。
  - `hero_alan_attack_0-4`（2026-07-27 整合）：Alan 的 `sword_horizontal_slash` 五幀動畫，由 John 選定並指定幀序，OpenAI 內建 imagegen 生成後以洋紅鍵去背、以腳底 bottom-center 錨點正規化。來源保留於 `assets-source/role/main/alan/battle_attack_strip_{raw,alpha}.png`、`battle_attack_0..4.png`、`battle_attack_review_montage.png`；提示詞作者 John／協作 Agent。
  ...
  - Marin 戰鬥 `attack / dagger_thrust` 五幀 strip（2026-07-23）：OpenAI 內建 imagegen 依已驗收的 Marin seed 與短刀 weapon reference 生成，經 John 驗收後以洋紅鍵去背、依 bottom-center root-lock 對齊並重排為「預備 → 回復預備 → 蓄力 → 左手突刺 → 收刀」。原始 strip、RGBA strip、逐幀來源與 Review montage 保存於 `assets-source/role/main/marin/battle_attack_strip_{raw,alpha}.png`、`battle_attack_0..4.png`、`battle_attack_review_montage.png`；尚未整合為 runtime 動畫。提示詞作者 John／協作 Agent。
