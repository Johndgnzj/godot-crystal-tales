...
- `assets/battle/hero_*.png`：除下列明列的 AI idle 例外外，由 LPC 合成角色幀放大裁切（沿用 LPC CC-BY-SA/GPL 授權），
  ...
  - `hero_marin_idle_0..3`（2026-08-11 更新）：Marin 3.5 頭身 `idle / calm_ready` 四幀循環，由 OpenAI 內建 imagegen 依已驗收 Seed 與短刀 reference 生成 Frame 0／2，Frame 1 以核可端點插值；正式循環為 `0 → 1 → 2 → 1`。John 手動完成三張 Alpha 邊緣微調後驗收，正式幀以 0.5× Nearest 正規化至 543×724、腳底 `y=628`。來源保存於 `assets-source/role/main/marin/battle_idle_0..3.png`、`battle_idle_strip_alpha.png` 與 `battle_idle_review.gif`；提示詞作者 John／協作 Agent。`hero_marin_attack_0-4` 仍為 2026-07-30 的舊二頭身素材，待 P2-2 重做。
