...
## 六、map-def.xlsx 維護規範（連通關係的權威來源）

`map-def.xlsx` 是每個地區「螢幕格編號＋連通關係」的權威表，接圖時依它排出入口與鄰接。

**檔案結構**
- 每地區一張工作表，命名 `<地區名>-<代號>`（如 `東之森深處-M4`）。
- 總表 `工作表1` ＝ `代號｜地區名稱｜檔案前綴`（新地區要在此加一列）。

**grid 記法**
- 格子擺在格點上；**橫向相鄰用 `ー`(U+30FC)、縱向相鄰用 `|`(U+007C)** 標示連通。
- **沒有連線符號＝不相連**（相鄰不代表連通，這是刻意的）。
- **跨區出口用區域代號當端點**（如 `L7=M4` 表示該格往該方向出到 M4）。
- 每張表 A1 標 `簡易地圖`，B1/C1 可記格數（如 `14` `格`）。

**儲存格格式（一律，2026-07-17 John 定案）**
- **高度、寬度一律 26px、水平＋垂直置中**（讓 grid 看起來像方格地圖）。
- openpyxl 對應值：`row height = 19.5`（pt，＝26px @96dpi）、`col width ≈ 3.0`（字元，≈26px）、`Alignment(horizontal="center", vertical="center")`；可用 `ws.sheet_format.defaultRowHeight/defaultColWidth` 設預設。
- 新工作表照此設定。

**設計原則（探索感，仿 M2）**
- 用**迴圈**（cycle）、**三岔口**（3 出口）、**死路藏寶箱**做出探索感；避免整張一路單向直線。房間出口數要有變化（1/2/3 口混用）。
