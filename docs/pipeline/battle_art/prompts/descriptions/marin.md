...
- 比例狀態: **Seed 已驗收**；158 cm、約 3.5 頭身，已通過 G1／G2 與五人等主體高度／設定身高 lineup。舊 idle／hurt／cast／death／attack 仍屬二頭身失效素材，必須全套重做
...
- 最後一版: 2026-08-13 的 3.5 頭身 Seed、`idle / calm_ready`、`hurt / recoil_guard`、`cast / offhand_rear_cast` 與 `death / fall_forward` 已驗收；Idle 已完成 G7，hurt／cast／death 已通過 G5、待瑪琳全套完成後統一進入 G7，目前只剩 attack。Idle 正式來源為 John 手動去背的三張 `finetune`，四幀循環 `0 → 1 → 2 → 1`、每幀 0.18 秒、543×724、腳底 `y=628`；正式 PNG 保留手動 Alpha，GIF 僅因格式限制採二值透明。舊二頭身 idle／hurt／cast／death／attack 僅保留作為動作方向與錯誤排查參考。
