...
hub 一個入口用漢堡選單整合下面四個網頁工具，同一視窗切換（不用開四次）。

## 各目錄

| 目錄 | 用途 | 單獨啟動（可選）|
|---|---|---|
| `hub/` | **統一入口**：漢堡選單整合下面四個工具（`start.sh` 起的就是它）| `python3 tools/hub/serve.py` → `:8800` |
...
各工具的操作說明**寫在工具網頁內**。地圖工具另有完整設計手冊：[`docs/pipeline/地圖製作流程.md`](../docs/pipeline/地圖製作流程.md)。
